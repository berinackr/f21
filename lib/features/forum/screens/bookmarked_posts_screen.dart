...
class BookmarkedPostsScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<ConsumerStatefulWidget> createState() => _BookmarkedPostsScreenState();
}
...
class _BookmarkedPostsScreenState extends ConsumerState<BookmarkedPostsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: CustomScrollView(controller: scrollController, restorationId: "forum_feed_screen", slivers: [
        Consumer(
                ...
                return posts.isEmpty
                    ? const SliverToBoxAdapter(
                        child: Column(children: [
                          SizedBox(height: 20),
                          Text("Kaydedilen paylaşımlarınız bulunmamaktadır!"),
                        ]),
                      )
                    : SliverList(
...
class Post extends ConsumerWidget {
  ...
  void upvotePost(BuildContext context, WidgetRef ref) {
    ...
    if (user != null) {
      ref.read(forumControllerProvider.notifier).upvotePost(post, context, index, isMostLiked, true);
    } else {
  ...
  void downvotePost(BuildContext context, WidgetRef ref) {
    final user = ref.read(userProvider);
    if (user != null) {
      ref.read(forumControllerProvider.notifier).downvotePost(post, context, index, isMostLiked, true);
    } else {
      showSnackBar(context, "Önce giriş yapmalısınız!");
    }
  }
  ...
  void bookmarkPost(BuildContext context, WidgetRef ref) {
    final user = ref.read(userProvider);
    if (user != null) {
      ref.read(forumControllerProvider.notifier).bookmarkPost(post, context, index, isMostLiked, true);
    } else {
      showSnackBar(context, "Önce giriş yapmalısınız!");
    }
  }
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return SliverToBoxAdapter(
      child: state.maybeWhen(
          orElse: () => const SizedBox.shrink(),
          data: (items) {
            final noMoreItems = ref.read(bookmarkedPostsProvider(id).notifier).noMoreItems;
            return noMoreItems
                ? items.isEmpty
